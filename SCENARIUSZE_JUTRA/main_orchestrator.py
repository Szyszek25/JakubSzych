...
from config import (
    ...
    TEMPERATURE_REALISTIC, OLLAMA_MODEL, OLLAMA_BASE_URL,
...
class ScenarioOrchestrator:
    ...
    def __init__(self, config: Dict, openai_api_key: str = "", gemini_model=None):
        self.config = config
        self.openai_api_key = openai_api_key  # Opcjonalne
        self.gemini_model = gemini_model  # Opcjonalne
        # Używamy lokalnego LLM (Ollama)
    ...
    def run_full_analysis(
        ...
        logger.info("\n[KROK 3] Analiza danych...")
        data_analyzer = DataAnalyzer(self.config, self.openai_api_key)  # openai_api_key opcjonalne
        ...
        # Ograniczenie rekomendacji do najważniejszych (max 5-7)
        from report_editor import ReportEditor
        editor = ReportEditor()
        limited_recommendations = editor.limit_recommendations(recommendations, max_per_category=5)
        
        # Dodanie rekomendacji do raportu
        recommendations_text = self.recommendation_engine.format_recommendations_for_report(limited_recommendations)
        final_report += "\n\n" + recommendations_text
        
        # Redakcja raportu do wersji decyzyjnej (dla MSZ)
        final_report_edited = editor.edit_report(final_report)
        final_report_edited = editor.add_executive_summary(final_report_edited)
        ...
        return {
            ...
            "report": final_report_edited,  # Używamy zredagowanej wersji
            "report_raw": final_report,  # Surowa wersja dla demo
            "statistics": {
    ...
    def _get_demo_data(self) -> List[DataSource]:
        ...
        # Realistyczne dane demo związane z czynnikami sytuacyjnymi
        demo_sources = [
            DataSource(
                url="https://example.com/demo1",
                title="Raport: Przemysł procesorów graficznych po katastrofie naturalnej",
                content="""
                Wiodący światowy producent procesorów graficznych doświadczył katastrofy naturalnej 
                przed miesiącem, co spowodowało utratę 60% zdolności produkcyjnych. 
                Odbudowa mocy produkcyjnych poprzez inwestycje w filie zlokalizowane na obszarach 
                nieobjętych katastrofą potrwa do końca roku 2028. 
                Wpływa to na globalny łańcuch dostaw elektroniki i może prowadzić do wzrostu cen 
                oraz opóźnień w dostawach komponentów dla przemysłu motoryzacyjnego i ICT.
                """,
                date=datetime.now(),
            ...
            DataSource(
                ...
                title="Analiza: Przemysł motoryzacyjny w Europie",
                content="""
                Przemysł motoryzacyjny w Europie bardzo wolno przestawia się na produkcję 
                samochodów elektrycznych. Rynek europejski zalewają tanie samochody elektryczne 
                z Azji Wschodniej. Europejski przemysł motoryzacyjny będzie miał w roku 2025 
                zyski na poziomie 30% średnich rocznych zysków z lat 2020-2024. 
                To może prowadzić do redukcji zatrudnienia i restrukturyzacji sektora.
                """,
                date=datetime.now(),
                source_type="institution",
                language="en"
            ...
            DataSource(
                ...
                title="Prognoza PKB: Strefa euro w 2025",
                content="""
                PKB krajów strefy euro w roku 2025 spadnie średnio o 1,5% w stosunku do roku 2024. 
                Główne przyczyny to spowolnienie w przemyśle motoryzacyjnym, zakłócenia w łańcuchach 
                dostaw oraz niepewność geopolityczna. Wpływa to na budżety państw członkowskich 
                i może wymagać dodatkowych środków z funduszy unijnych.
                """,
                date=datetime.now(),
                source_type="institution",
                language="en"
            ...
            DataSource(
                ...
                title="Sytuacja na Ukrainie: Rozejm i gospodarka",
                content="""
                Na wschodzie Ukrainy trwa słaby rozejm. Rosja kontroluje dwie główne elektrownie 
                ukraińskie, które pracują na potrzeby konsumentów rosyjskich. 
                Gospodarka ukraińska rozwija się w tempie 4% PKB, głównie dzięki inwestycjom 
                w przemysł zbrojeniowy i odbudowę infrastruktury. Inwestycje amerykańskie 
                kierowane są do przemysłu wydobywczego (surowce krytyczne).
                """,
                date=datetime.now(),
                source_type="institution",
                language="en"
            ...
            DataSource(
                ...
                title="Inwestycje UE w Ukrainie",
                content="""
                Roczne inwestycje UE w Ukrainie są na poziomie 3% ukraińskiego PKB i utrzymają się 
                na takim poziomie do roku 2029. Inwestycje koncentrują się na infrastrukturze, 
                energetyce i przemyśle zbrojeniowym. Wsparcie finansowe UE jest kluczowe 
                dla stabilności gospodarczej Ukrainy.
                """,
                date=datetime.now(),
                source_type="institution",
                language="en"
            ...
            DataSource(
                url="https://example.com/demo6",
                title="Energia: OZE i paliwa węglowodorowe",
                content="""
                Gwałtowny wzrost udziału energii z OZE w miksie energetycznym krajów UE oraz Chin 
                od początku roku 2028. W połowie roku 2023 średniej wielkości kraj południowoamerykański 
                odkrył ogromne i łatwe do eksploatacji złoża ropy naftowej i gazu ziemnego 
                dorównujące wielkością złożom Arabii Saudyjskiej i Kataru. 
                To przełoży się pod koniec roku 2027 na nadpodaż tych paliw na światowe rynki. 
                Wzrost podaży energii z OZE oraz nadpodaż paliw węglowodorowych przekładają się 
                na znaczny spadek cen ropy: do poziomu 30-35 USD za baryłkę. 
                Będzie to miało wpływ na budżet Rosji oraz innych krajów producentów ropy.
                """,
                date=datetime.now(),
                source_type="institution",
                language="en"
            ...
            DataSource(
                url="https://example.com/demo7",
                title="Analiza geopolityczna: Wpływ na państwa regionu",
                content="""
                Sytuacja międzynarodowa wpływa na państwa regionu Morza Bałtyckiego. 
                Państwa członkowskie UE i NATO monitorują rozwój sytuacji w Ukrainie, 
                zmiany w przemyśle motoryzacyjnym oraz wahania cen surowców energetycznych. 
                Wymaga to koordynacji działań na poziomie unijnym i sojuszniczym.
                """,
                date=datetime.now(),
                source_type="institution",
                language="en"
    ...
    def update_weights_and_recalculate(
        ...
        for factor_id, new_weight in new_weights.items():
            old_factor = self.reasoning_engine.weighted_factors.get(factor_id, {})
            old_weight = old_factor.get('weight', 0) if isinstance(old_factor, dict) else (getattr(old_factor, 'weight', 0) if hasattr(old_factor, 'weight') else 0)
...
if __name__ == "__main__":
    ...
    report_file = os.path.join(reports_dir, f"raport_atlantis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
